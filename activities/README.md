# Activities

这里整理 `VIRTURA` 当前对外开放的活动、展映、测试与公开说明。

如果你是从小红书、朋友转发、活动海报或某一页 GitHub 链接来到这里，这页应该先回答一件很简单的事：

最近到底有什么真的在发生，我应该先从哪里跟。

## 推荐先做的一件事

如果你想稳定地跟着我们的公开活动走，最直接的方式是先订阅：

- [VIRTURA 官方活动日历](https://lu.ma/VIRTURA)

这是我们当前最重要的公共活动入口。新的茶话会、线上演出空间测试、展映、公开说明和观看入口开放，都会优先从这里继续发布。

## 我们现在主要做哪几类活动

目前公开活动大致分成 5 类：

1. `Space Salon / 数字沙龙 / 交流客厅`
   更偏社群交流、嘉宾分享、一起放东西看、一起聊最近在做什么。
2. `太空气球 / Baloon Live Space`
   更偏线上 live set、音画实验、演出空间测试和共测夜。
3. `MOVA / Museum of Virtual Art`
   更偏线上展映、作品观看入口和轻量数字展览。
4. `在制内容审计会`
   更偏把正在做的页面、视觉、声音、空间内容拿出来，一起挑问题。
5. `Digital Library / Digital Stage 公开说明`
   更偏围绕仓库、归档、发布结构和 AI 时代的内容管理，做短而实用的公开说明。

另外还会穿插一些 `观看入口开放`、`页面上新`、`公开测试` 这类更轻的节点。

## 为什么我们持续做公共活动

这些活动对我们来说，不只是宣传动作。

它更像一个长期开放的公共前台。有人会从这里认识团队，有人会只是来旁听一次，有人是想看看最近有什么新作品，有人则是想慢慢判断自己是不是愿意继续交流或合作。

所以这里不会默认每个人都要加入我们。你也完全可以只是订阅日历，偶尔来一场，看看最近在发生什么。

## 当前公开活动入口

- [VIRTURA 官方活动日历](https://lu.ma/VIRTURA)
- [VIRTURA-SpacePort / stations](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations)
- [VIRTURA 空间茶话会 Vol.0｜带上一段视觉，来客厅聊聊存档与远方](https://luma.com/iehkdjr0)
- [太空气球线上演出空间 Alpha 测试](https://luma.com/go6kxnnw)

## 计划中的节点

如果你更关心“接下来会发生什么”，先看这里：

- [Planned / 计划中](./planned/README.md)

当前已经在主排期里的节点包括：

- `2026-03-29` `VIRTURA 空间茶话会 Vol.3｜AI 时代，我们怎么整理自己的作品、过程和知识`
- `2026-04-03` `MOVA 展映 01｜我们第一次把 GitHub 里的内容整理成数字展览`
- `2026-04-04` `在制内容审计会 01｜把正在做的东西拿出来，我们一起挑问题`
- `2026-04-10` `Digital Library / Digital Stage 公开说明 01｜AI 时代怎么把作品、材料和过程整理成能继续工作的库`
- `2026-04-11` `VIRTURA 空间茶话会 Vol.4｜带一段最近看到的东西来，我们一起放、一块聊`
- `2026-04-17` `发布与观看入口开放 01｜Drop Flow 版本观看室开放`
- `2026-04-18` `太空气球 Session 02｜线上 live set 试放与视觉共演夜`
- `2026-04-24` `Digital Library / Digital Stage 公开说明 02｜知识网络怎么整理成别人真的能用的入口`
- `2026-04-25` `VIRTURA 空间茶话会 Vol.5｜带一个最近还没讲顺的东西来，我们一起继续聊`
- `2026-05-01` `MOVA 展映 02｜观察与共生：扫描、展览与工作坊`
- `2026-05-02` `在制内容审计会 02｜网页、场景和版本，我们继续挑问题`
- `2026-05-08` `Digital Library / Digital Stage 公开说明 03｜网页前台、资料库和活动入口怎么接起来`
- `2026-05-09` `太空气球 Beta 共测夜｜第二轮线上演出空间联调`

目前已经公开的新增节点包括：

- [VIRTURA 空间茶话会 Vol.3｜AI 时代，我们怎么整理自己的作品、过程和知识](https://luma.com/jy5fef4u)
- [MOVA 展映 01｜我们第一次把 GitHub 里的内容整理成数字展览](https://luma.com/kw41rtby)
- [在制内容审计会 01｜把正在做的东西拿出来，我们一起挑问题](https://luma.com/3be89q8l)
- [Digital Library / Digital Stage 公开说明 01｜AI 时代怎么把作品、材料和过程整理成能继续工作的库](https://luma.com/kjyluf41)
- [VIRTURA 空间茶话会 Vol.4｜带一段最近看到的东西来，我们一起放、一块聊](https://luma.com/wqi71c4f)

## 往期公开节点

当前先把已经公开可确认的节点整理为第一版时间线：

| 日期 | 活动 | 形式 | 角色 |
|---|---|---|---|
| 2026.03.21 | [太空气球线上演出空间 Alpha 测试](https://luma.com/go6kxnnw) | 线上演出空间测试 / 公开申请中 | 太空气球 |
| 2026.03.14 | [VIRTURA 空间茶话会 Vol.1｜云端续章](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations/space-salon/vol-01-cloud-sequel) | 闭门线上分享 | Space Salon 01 |
| 2026.01.31 | [VIRTURA 空间茶话会 Vol.0｜带上一段视觉，来客厅聊聊存档与远方](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations/space-salon/vol-00-archive-and-distance) | 线下客厅沙龙 / 小规模聚会 | Space Salon 00 |
| 2026.01 | [愚见观池｜情感容器构建工作坊](https://github.com/ewanqian/portfolio/blob/main/workshops/202601-%E6%84%9A%E8%A7%81%E8%A7%82%E6%B1%A0-%E6%83%85%E6%84%9F%E5%AE%B9%E5%99%A8%E5%B7%A5%E4%BD%9C%E5%9D%8A.md) | offline workshop | 主讲 / 方法分享 |
| 2025.07 | [杭州中心美术馆｜巴别瓶公共活动](https://github.com/ewanqian/portfolio/blob/main/workshops/202507-%E6%9D%AD%E5%B7%9E%E4%B8%AD%E5%BF%83-AI-Agent%E5%B7%A5%E4%BD%9C%E5%9D%8A.md) | offline public activity | 工作坊导师 |
| 2025.05 | [线上 5 天直播工作坊](https://github.com/ewanqian/portfolio/blob/main/workshops/202505-%E7%BA%BF%E4%B8%8A-5%E5%A4%A9%E7%9B%B4%E6%92%AD%E5%B7%A5%E4%BD%9C%E5%9D%8A.md) | online workshop | 主讲 / 课程设计 |
| 2025.04 | [重新定义数字空间｜半成品沙龙](https://github.com/ewanqian/portfolio/blob/main/workshops/202504-%E5%B7%A8%E9%B9%BF%E8%B7%AF-%E5%8D%8A%E6%88%90%E5%93%81%E6%B2%99%E9%BE%99.md) | offline salon | 分享嘉宾 |
| 2024.12 | [AI+3D 空间交互艺术创作工坊](https://github.com/ewanqian/portfolio/blob/main/workshops/202412-%E6%BC%95%E6%B2%B3%E6%B3%BE-AI3D%E5%B7%A5%E5%9D%8A.md) | offline workshop | 特邀分享嘉宾 |

如果你想按“往期活动入口”单独看，见：

- [Past / 往期公开活动入口](./past/README.md)

## 你会在这个日历里看到什么

- `Space Salon / 数字沙龙 / 交流客厅`
- `太空气球` 的线上演出空间测试、live set 与共测夜
- `MOVA` 的展映开启、观看入口和主题小展
- `在制内容审计会`
- `Digital Library / Digital Stage` 相关公开说明
- 页面开放、观看入口开放和其他适合公开发布的节点

## 未来 8 周主线

接下来这段时间，我们更想把活动做成一条有起伏、有回访理由的时间线，而不是一组彼此无关的零散节点。

可以把它理解成：

- `3 条长期活动线`
  - `Space Salon`
  - `太空气球`
  - `MOVA`
- `2 类配套节点`
  - `在制内容审计会`
  - `Digital Library / Digital Stage 公开说明`

如果你只想先看“未来 1-2 个月会发生什么”，直接看：

- [Planned / 计划中](./planned/README.md)
- [未来 8 周活动路线图](../docs/eight-week-activity-roadmap-2026-03-16.md)

## 当前最清楚的 5 类活动与发布线

### 1. 太空气球 / Baloon Live Space

更偏线上演出空间、音乐与音画实验、公开测试和后续的共演节点。

目前公开可见的代表节点是：

- [太空气球线上演出空间 Alpha 测试](https://luma.com/go6kxnnw)

### 2. Space Salon / 数字沙龙 / 交流客厅

更偏线下分享会、线上漫谈、小规模交流、一起放内容看和邀请制嘉宾分享。

目前公开可见的代表节点是：

- [VIRTURA 空间茶话会 Vol.0](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations/space-salon/vol-00-archive-and-distance)
- [VIRTURA 空间茶话会 Vol.1](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations/space-salon/vol-01-cloud-sequel)

### 3. MOVA / Museum of Virtual Art

更偏线上展映、作品观看室、轻量展览，以及从 GitHub library 快速整理成公共观看入口的发布动作。

当前入口：

- [MOVA](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations/mova)

### 4. 在制内容审计会

更偏把正在做的页面、视觉、声音、空间内容和版本拿出来，一起看问题出在哪里。

这条线不会假装“大家都很温柔”，它更像一次认真、直接、对创作有帮助的互相批评。

### 5. Digital Library / Digital Stage 公开说明

更偏围绕仓库、归档、版本、页面前台和 AI 时代的内容管理，做短而实用的公开说明。

重点不是卖概念，而是把一套确实能继续工作的组织方法讲明白。

## Space Salon 这条活动线

当前最适合外部先认识的一条活动线，还是 `Space Salon / 数字沙龙`。

- 它承接的是更轻量、更能自然发生交流的场景
- 它既可以连接作品和研究，也可以连接后续的公共活动
- 它不默认你要先加入谁，很多人只是先来听一场、聊一会儿
- 更完整的系列时间线见 [VIRTURA-SpacePort / stations](https://github.com/ewanqian/VIRTURA-SpacePort/tree/main/stations)

## 日常还会从哪里同步

- 小红书：`EWANCT前宇文`
- 小红书：`VIRTURA`
- 小红书群与朋友转发链接

如果你已经在活动日历里订阅了我们，后续活动邮件会是最稳定的一条更新方式。

## 日历如何和 GitHub 配合

我们会尽量把日历和 GitHub 做成一套自然的前后衔接：

- `Luma` 负责订阅、报名、提醒和公开发生的那个时刻
- `VIRTURA-Collective` 负责团队、作品、活动和合作入口
- `VIRTURA-SpacePort` 负责知识网络、项目档案、活动归档和 `MOVA` 线上展映入口

所以一个节点可以不是“来现场见面”，也可以是：

- 一个线上展映正式开启
- 一个观看入口开放
- 一个公开测试开放申请
- 一次围绕归档与发布结构的公开说明

## 为什么这里不是一堆独立活动页面

`VIRTURA-Collective/activities` 现在不再试图把每一期活动都拆成很多分散页面。

现在更明确的分工是：

- 这里负责总入口、时间线、计划中节点和继续进入的方向
- `VIRTURA-SpacePort/stations` 负责更完整的活动站点、系列归档和发生现场
- `portfolio/workshops` 负责一些已经发生过的公开工作坊和历史条目

所以这里看起来比以前更干净，不是因为内容没了，而是因为过去分散的活动内容被收回到更适合长期承接它们的位置了。

## 人通常怎么靠近这个网络

很多关系都不是从正式“加入申请”开始，而是从这些活动场景进入：

1. 先旁听一次公开活动或线上分享
2. 再对某个系列、某个研究方向或某个小模块产生兴趣
3. 最后根据节奏，决定是继续关注、参与一次具体协作，还是长期保持联系

这也是为什么活动页很重要：它不只服务于合作，也服务于长期关注这个网络的人。

## 如果你想进一步参与

目前更适合的进入方式有 4 种：

1. 围绕某个项目系列参与一次清晰的协作
2. 围绕研究、写作或知识网络贡献一个小模块
3. 围绕工具、模板或发布结构参与搭建
4. 先从活动、沙龙和公开交流建立连接

更具体的参与说明见：[Collaboration](../collaboration/README.md)

## 后续继续补什么

这页后续优先补：

- 更多线上线下节点
- 活动与作品、研究、发布之间的互链
- 统一的活动元数据：`date / city / format / related project / role`
